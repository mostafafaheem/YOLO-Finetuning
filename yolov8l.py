...
image = (
    ...
    .uv_pip_install(
        ["ultralytics~=8.3.232", "opencv-python~=4.10.0", "term-image==0.7.1"]
    )
)
...
def main():
    with volume.batch_upload() as batch:
        batch.put_file("config.yaml", "/data/data/config.yaml")
        batch.put_directory("data/data", "/data/data")
